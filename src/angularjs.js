...
export class AngularJsAdapter extends Adapter implements AdapterInterface {
  ...
  getDependencyInjections(target: any): Array<string> {
    if (target === null) {
      ...
      if (
        this.injects.hasOwnProperty(target) &&
        Array.isArray(this.injects[target])
      ) {
  ...
  getDependencyInjectionParams(target: any): Array<string> {
    if (typeof target === 'function') {
      if (target.length > 0) {
        return target.toString().replace(/^function [a-z0-9]+\(([a-z0-9 ,$]+)(.*(\r?\n)?)*/i, '$1').split(', ');
      } else {
  ...
  toCamelCase(string: string): string {
    return string
      .replace(/\-(.)/g, ($1: string): string => { // eslint-disable-line no-useless-escape
        return $1.toUpperCase();
      })
      .replace(/\-/g, '') // eslint-disable-line no-useless-escape
      .replace(/^(.)/, ($1: string): string => {
      ...
      return (target: Function): any => {
        ...
        if (indexOf < 0) {
          ...
          if (length !== target.$inject.length) {
            console.warn('Dependency-Injection-Length does not match Annotation-Length!', target.name, length, target.$inject, target); // eslint-disable-line no-console
          }
          if (
            this.getNodeEnv() === 'Development' &&
            this.compareArrays(target.$inject, params) === false
          ) {
            console.warn('Dependency-Injection-Order does not match Annotation-Order!', target.name, target.$inject, params); // eslint-disable-line no-console
          }
